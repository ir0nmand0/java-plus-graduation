...
public interface PrivateUserRequestApi {
    ...
    List<RequestDto> findAllByIdInAndEventId(@PathVariable(required = false) @Positive long userId,
    ...
    @ResponseStatus(HttpStatus.CREATED)
    void saveAll(@PathVariable(required = false) @Positive long userId, @RequestBody @NotEmpty List<RequestDto> requests);
}
